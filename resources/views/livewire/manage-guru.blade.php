    ...
    <div class="bg-white p-6 rounded-lg shadow-lg mb-6">
        <form wire:submit.prevent="save">
            ...
            <div class="mb-4">
                ...
                <input type="text" wire:model="nip" class="w-full p-2 border rounded">
                @error('nip') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            <div class="mb-4">
                <label class="block text-gray-700">Mata Pelajaran</label>
                <input type="text" wire:model="mata_pelajaran" class="w-full p-2 border rounded">
                @error('mata_pelajaran') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            <div class="mb-4">
                <label class="block text-gray-700">Kelas</label>
                <select wire:model="kelas_ids" multiple class="w-full p-2 border rounded">
                    @foreach($kelas as $k)
                        <option value="{{ $k->id }}">{{ $k->nama_kelas }}</option>
                    @endforeach
                </select>
                @error('kelas_ids') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            ...
            <tbody>
                @forelse($guru as $g)
                    <tr>
                        ...
                        <td class="border p-2">
                            <button wire:click="edit({{ $g->id }})" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">Edit</button>
                            <button wire:click="delete({{ $g->id }})" class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">Hapus</button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="border p-2 text-center">Belum ada data guru.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
